...
class ConfigField(Vertical):
    ...
    def compose(self) -> ComposeResult:
        ...
        # Créer un conteneur vertical pour le champ
        with Vertical():
            # Ligne du haut avec le label et l'étoile si requis
            with Horizontal():
...
class PluginConfigContainer(Vertical):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id=f"fields_{self.plugin_id}", classes="fields-container"):
...
class PluginConfig(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="plugin-config-container"):
            ...
            for plugin, instance_number in self.plugins:
                ...
                plugin_container = self._create_plugin_config(plugin, instance_number)
    ...
    def _create_plugin_config(self, plugin: str, instance_number: int) -> Widget:
        """Create configuration fields for a plugin"""
        logger.debug(f"Creating config for plugin {plugin} #{instance_number}")
        settings_path = os.path.join(os.path.dirname(__file__), '..', 'plugins', plugin, 'settings.yml')
        try:
            with open(settings_path, 'r') as f:
                settings = yaml.safe_load(f)
                logger.debug(f"Loaded settings for {plugin}: {settings}")
        except Exception as e:
        ...
        elif instance_number not in self.fields_by_plugin[plugin]:
            self.fields_by_plugin[plugin][instance_number] = {}
        fields_by_id = {}
        ...
        config_fields = []
        fields_dict = settings.get('config_fields', {})
        logger.debug(f"Config fields from settings: {fields_dict}")
        
        for field_id, field_config in fields_dict.items():
            field_config['id'] = field_id
            config_fields.append(field_config)
            
        logger.debug(f"Converted config fields: {config_fields}")
