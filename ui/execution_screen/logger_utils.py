...
if not logger.handlers:
    ...
    formatter = logging.FileHandler('/tmp/logger_utils.log') if os.path.exists('/tmp') else logging.StreamHandler()
...
class LoggerUtils:
    ...
    _priority_queue: Deque[Message] = deque(maxlen=100)
    ...
    async def _periodic_logs_display(app):
        ...
        try:
            ...
            logger.info("Démarrage de la boucle de traitement des logs")
                ...
                try:
                    ...
                    if (queue_size > 0 and (should_flush or time_to_flush)) or force_flush:
                        try:
                            ...
                            LoggerUtils._priority_queue.clear()
                            last_flush_time = time.monotonic()
    ...
    async def start_logs_timer(cls, app):
        ...
        try:
            ...
            logger.info("Timer de logs démarré avec succès")
        except Exception as e:
    ...
    async def process_output_line(cls, app, line: str, plugin_widget=None, 
        ...
        Args:
            ...
            is_priority: Si True, le message est traité en priorité
        """
        ...
        if message_obj and message_obj.type in [MessageType.PROGRESS, MessageType.PROGRESS_TEXT]:
            ...
            if needs_queue:
                queue = cls._priority_queue if is_priority else cls._pending_messages
        ...
        if message_obj:
            ...
            if needs_queue:
                ...
                await cls.display_message(app, message_obj, is_priority)

    @classmethod
    async def display_message(cls, app, message_obj: Message, is_priority: bool = False):
        """
        ...
        Args:
            ...
            is_priority: Si True, le message est prioritaire
        """
        if not TEXTUAL_AVAILABLE:
            return
    ...
    async def flush_pending_messages(cls, app):
        ...
        try:
            ...
            messages_to_process = []
            max_messages = 50  # Limite de sécurité pour éviter les surcharges d'UI
            
            # Priorité aux messages prioritaires
            try:
                while cls._priority_queue and len(messages_to_process) < max_messages:
                    messages_to_process.append(cls._priority_queue.popleft())
            except Exception as e:
                logger.error(f"Erreur extraction queue prioritaire: {e}")
                cls._priority_queue.clear()  # Vider en cas d'erreur
    ...
    async def add_log(cls, app, message: str, level: str = "info", target_ip: Optional[str] = None):
        ...
        try:
            ...
            # Déterminer si c'est un message prioritaire
    ...
    def get_pending_message_count(cls) -> int:
        ...
        """
        return len(cls._pending_messages) + len(cls._priority_queue)