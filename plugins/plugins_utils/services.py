...
class ServiceCommands(PluginUtilsBase):
    ...
    def _find_systemctl(self) -> Optional[str]:
        ...
        for path in ['/bin/systemctl', '/usr/bin/systemctl', '/sbin/systemctl', '/usr/sbin/systemctl']:
            ...
            if success:
                self.log_debug(f"Exécutable systemctl trouvé: {path}")
                return path
    ...
    def _run_systemctl(self, args: List[str], check: bool = False, needs_sudo: bool = True, **kwargs) -> Tuple[bool, str, str]:
        ...
        if not self._systemctl_path:
            ...
            cmd = ['systemctl'] + args
            self.log_warning("Chemin systemctl non trouvé, tentative d'exécution directe.")
        else:
    ...
    def start(self, service_name: str) -> bool:
        ...
        if success:
            ...
            if "already running" in stderr.lower() or "job is running" in stderr.lower() or "service is already active" in stderr.lower():
                 self.log_info(f"Le service {service_name} était déjà en cours d'exécution.")
                 # Vérifier l'état actif pour être sûr
                 return self.is_active(service_name)
            self.log_error(f"Échec du démarrage du service {service_name}. Stderr: {stderr}")
            return False
    ...
    def stop(self, service_name: str) -> bool:
        ...
        if success:
            ...
            if "not running" in stderr.lower() or "not loaded" in stderr.lower() or "inactive" in stderr.lower():
                 self.log_info(f"Le service {service_name} était déjà arrêté ou non chargé.")
                 # Vérifier l'état inactif
                 return not self.is_active(service_name)
            self.log_error(f"Échec de l'arrêt du service {service_name}. Stderr: {stderr}")
            return False
    ...
    def restart(self, service_name: str) -> bool:
        ...
        if success:
            ...
            if self.is_active(service_name):
                 ...
                 self.log_error(f"Stderr (restart): {stderr}") # Afficher stderr même si code 0
                 return False
        else:
            self.log_error(f"Échec du redémarrage du service {service_name}. Stderr: {stderr}")
            return False
    ...
    def reload(self, service_name: str) -> bool:
        ...
        if success:
            ...
            if success_ror:
                ...
                self.log_error(f"Échec initial de reload pour {service_name}. Stderr: {stderr}")
                self.log_error(f"Échec de reload-or-restart pour {service_name}. Stderr: {stderr_ror}")
                return False

    def enable(self, service_name: str, now: bool = False) -> bool:
        """
        ...
        self.log_info(f"{action} du service au démarrage: {service_name}")
        cmd = ['enable']
        if now:
            cmd.append('--now')
        ...
        if success:
            ...
            if "already enabled" in stderr.lower():
                 self.log_info(f"Le service {service_name} était déjà activé.")
    ...
    def disable(self, service_name: str, now: bool = False) -> bool:
        """
    ...
    def is_active(self, service_name: str) -> bool:
        ...
        self.log_debug(f"Service {service_name} est actif: {is_act}")
        return is_act
    ...
    def is_enabled(self, service_name: str) -> bool:
        ...
        self.log_debug(f"Service {service_name} est activé au démarrage: {is_enb}")
        ...
        if not success and stderr and not re.search(r'(disabled|static|masked)', stderr, re.IGNORECASE):
             self.log_warning(f"Erreur lors de la vérification de l'état enabled pour {service_name}: {stderr.strip()}")
        return is_enb
    ...
    def get_status(self, service_name: str) -> Dict[str, Any]:
        ...
        """
        self.log_debug(f"Récupération du statut détaillé du service: {service_name}")
        ...
        if not success:
            ...
            self.log_error(f"Impossible d'obtenir le statut détaillé de {service_name}.")
            # Afficher les deux erreurs si disponibles
            if stderr: self.log_error(f"Stderr (show): {stderr}")
            if status_stderr: self.log_error(f"Stderr (status): {status_stderr}")
        ...
        try:
            ...
            for line in stdout.splitlines():
                 if line.strip(): # Ignorer les lignes vides
                      try:
                           ...
                           self.log_warning(f"Impossible de parser la ligne JSON du statut: {line}")
            ...
            self.log_debug(f"Statut détaillé pour {service_name}: {info}")
            return info

        except ImportError:
             self.log_error("Le module 'json' est nécessaire pour parser la sortie de systemctl show.")
             return {}
