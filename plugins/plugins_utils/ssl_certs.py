...
class SslCertCommands(PluginUtilsBase):
    ...
    def _parse_openssl_date(self, date_str: str) -> Optional[datetime]:
        ...
        try:
            ...
            self.log_warning(f"Impossible de parser la date openssl '{date_str}': {e}")
            return None
    ...
    def check_cert_expiry(self,
                          ...
                          port: int = 443,
                          days_warning: int = 30) -> Tuple[str, Optional[datetime], Optional[int]]:
        """
        ...
        self.log_info(f"Vérification de l'expiration du certificat pour: {target}")
        ...
        if cert_path:
            ...
            if not cert_path_obj.is_file():
                self.log_error(f"Fichier certificat non trouvé: {cert_path}")
            ...
            if not s_success or 'BEGIN CERTIFICATE' not in s_stdout:
                 self.log_error(f"Impossible de récupérer le certificat de {target}. Stderr: {s_stderr}")
                 if "connect:errno=" in s_stderr: self.log_error("  -> Vérifier connectivité et pare-feu.")
                 if "getaddrinfo: Name or service not known" in s_stderr: self.log_error("  -> Hôte ou port invalide.")
            ...
            self.log_error("Il faut fournir soit cert_path soit host.")
            return "ERROR", None, None
        ...
        if not success:
            self.log_error(f"Échec de la commande openssl pour récupérer la date d'expiration. Stderr: {stderr}")
            return "ERROR", None, None

        # Parser la date
        expiry_date = self._parse_openssl_date(stdout.strip())
        if not expiry_date:
            self.log_error(f"Impossible de parser la date d'expiration: {stdout.strip()}")
            return "ERROR", None, None
        ...
        if days_left < 0:
            status = "EXPIRED"
            self.log_error(f"Certificat pour {target} a EXPIRÉ le {expiry_date.strftime('%Y-%m-%d')}.")
        elif days_left < days_warning:
            status = "WARNING"
            self.log_warning(f"Certificat pour {target} expire bientôt ({days_left} jours restants, le {expiry_date.strftime('%Y-%m-%d')}). Seuil={days_warning}j.")
        else:
            status = "OK"
            self.log_info(f"Certificat pour {target} est valide jusqu'au {expiry_date.strftime('%Y-%m-%d')} ({days_left} jours restants).")
    ...
    def get_cert_info(self,
                      cert_path: Optional[Union[str, Path]] = None,
                      host: Optional[str] = None,
                      port: int = 443) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        target = str(cert_path) if cert_path else f"{host}:{port}"
        self.log_info(f"Récupération des informations du certificat pour: {target}")
        ...
        if cert_path:
            ...
            try:
                ...
                input_data = cert_data
                 ...
                 self.log_error(f"Erreur lors de la lecture de {cert_path}: {e}")
                 return None
        ...
        if not success:
            self.log_error(f"Échec de la commande openssl pour récupérer les informations. Stderr: {stderr}")
            return None
        ...
        self.log_debug(f"Informations certificat pour {target}: {info}")
        return info
    ...
    def generate_self_signed_cert(self,
        ...
        cert_path_obj = Path(cert_path)
        self.log_info(f"Génération d'un certificat auto-signé pour '{common_name}'")
        self.log_info(f"  Clé privée: {key_path_obj}")
        self.log_info(f"  Certificat: {cert_path_obj} (Valide {days} jours)")

        if not overwrite and (key_path_obj.exists() or cert_path_obj.exists()):
            self.log_error(f"Les fichiers de clé ou de certificat existent déjà. Utiliser overwrite=True.")
            return False
        ...
        try:
            ...
            self.log_error(f"Impossible de créer les dossiers parents: {e}")
            return False

        # 1. Générer la clé privée
        self.log_info("Génération de la clé privée...")
        if key_type.lower() == 'rsa':
            cmd_genkey = ['openssl', 'genpkey', '-algorithm', 'RSA',
             ...
             self.log_error(f"Type de clé non supporté: {key_type}. Utiliser 'rsa' ou 'ec'.")
             return False
        ...
        if not success_key:
            self.log_error(f"Échec de la génération de la clé privée. Stderr: {stderr_key}")
            if key_path_obj.exists(): key_path_obj.unlink() # Nettoyage
            return False
        self.log_success("Clé privée générée.")

        # 2. Générer le certificat auto-signé directement (sans CSR séparé)
        self.log_info("Génération du certificat auto-signé...")
        # Construire le sujet
        ...
        if success_cert:
            self.log_success(f"Certificat auto-signé généré: {cert_path_obj}")
            # Définir permissions sur le certificat (ex: 644)
            self.set_permissions(cert_path_obj, mode="644")
            return True
        else:
            self.log_error(f"Échec de la génération du certificat. Stderr: {stderr_cert}")
            # Nettoyage
            if key_path_obj.exists(): key_path_obj.unlink()
            if cert_path_obj.exists(): cert_path_obj.unlink()
            return False

    def verify_cert_chain(self, cert_path: Union[str, Path], ca_bundle_path: Union[str, Path]) -> bool:
        """
        ...
        self.log_info(f"Vérification de la chaîne de confiance pour {cert_path_obj} avec {ca_bundle_path_obj}")

        if not cert_path_obj.is_file():
            self.log_error(f"Fichier certificat introuvable: {cert_path_obj}")
            return False
        if not ca_bundle_path_obj.is_file():
            self.log_error(f"Fichier CA bundle introuvable: {ca_bundle_path_obj}")
            return False
        ...
        if success:
             ...
             if "OK" in stdout:
                  self.log_success(f"La vérification du certificat {cert_path_obj} a réussi.")
                  return True
             else:
                  # Succès mais sortie inattendue? Log et considérer comme échec prudent
                  self.log_warning(f"openssl verify a retourné 0 mais la sortie est inattendue: {stdout}")
                  return False
        else:
