...
class SecurityCommands(PluginsUtilsBase):
    ...
    def _ensure_ssh_dir(self, username: str, ssh_dir: Path) -> bool:
        ...
        try:
            ...
            if not ssh_dir.exists():
                self.log_debug(f"Création du dossier {ssh_dir}")
    ...
    def generate_ssh_key(self,
        ...
        self.log_debug(f"Génération de la clé SSH ({key_type}, {bits} bits) vers: {key_path_obj}")
        ...
        if success and key_path_obj.exists() and key_pub_path.exists():
            self.log_debug(f"Clé SSH générée avec succès: {key_path_obj}")
    ...
    def add_authorized_key(self, username: str, public_key_content: str) -> bool:
        ...
        # (Code inchangé)
        self.log_debug(f"Ajout d'une clé publique autorisée pour l'utilisateur: {username}")
        ...
        if key_exists:
            self.log_debug(f"La clé publique existe déjà dans {auth_keys_path}.")
            return True
        self.log_debug(f"Ajout de la clé à {auth_keys_path}")
        ...
        try:
            ...
            if not success_chown or not success_chmod:
                 ...
                 return False
            self.log_debug(f"Clé publique ajoutée et permissions définies pour {username}.")
            return True
        except KeyError:
    ...
    def remove_authorized_key(self, username: str, key_identifier: str) -> bool:
        ...
        # (Code inchangé)
        self.log_debug(f"Suppression de la clé publique autorisée pour '{username}' identifiée par '{key_identifier[:30]}...'")
        ssh_dir = self._get_user_home_ssh_dir(username)
        if not ssh_dir: return False
        auth_keys_path = ssh_dir / "authorized_keys"
        ...
        if success:
            self.log_debug(f"Clé(s) correspondant à '{key_identifier[:30]}...' supprimée(s) de {auth_keys_path} (si elle existait).")
            return True
        else:
    ...
    def set_permissions(self, path: Union[str, Path], mode: Union[str, int], recursive: bool = False) -> bool:
        ...
        mode_str = str(mode)
        self.log_debug(f"Modification des permissions de {target_path} en {mode_str}{' (récursif)' if recursive else ''}")
        ...
        if success:
            self.log_debug(f"Permissions de {target_path} mises à jour.")
            return True
        else:
    ...
    def set_ownership(self, path: Union[str, Path], user: Optional[str] = None, group: Optional[str] = None, recursive: bool = False) -> bool:
        ...
        if not owner_spec or owner_spec == ":":
             ...
             return False
        self.log_debug(f"Modification du propriétaire/groupe de {target_path} en {owner_spec}{' (récursif)' if recursive else ''}")
        if not target_path.exists():
             s_exists, _, _ = self.run(['test', '-e', str(target_path)], check=False, needs_sudo=True)
             if not s_exists:
                  self.log_error(f"Le chemin n'existe pas: {target_path}")
        ...
        success, stdout, stderr = self.run(cmd, check=False, needs_sudo=True)
        if success:
            self.log_debug(f"Propriétaire/groupe de {target_path} mis à jour.")
            return True
        else:
    ...
    def fail2ban_ban_ip(self, jail: str, ip_address: str) -> bool:
        ...
        # (Code inchangé)
        self.log_debug(f"Bannissement de l'IP {ip_address} dans la jail '{jail}' (fail2ban)")
        cmd = ['fail2ban-client', 'set', jail, 'banip', ip_address]
        success, stdout, stderr = self.run(cmd, check=False, needs_sudo=True)
        if success:
            if stdout: self.log_debug(f"Sortie fail2ban-client: {stdout.strip()}")
            self.log_debug(f"IP {ip_address} bannie dans la jail '{jail}'.")
            return True
        else:
    ...
    def fail2ban_unban_ip(self, jail: str, ip_address: str) -> bool:
        ...
        # (Code inchangé)
        self.log_debug(f"Débannissement de l'IP {ip_address} de la jail '{jail}' (fail2ban)")
        cmd = ['fail2ban-client', 'set', jail, 'unbanip', ip_address]
        success, stdout, stderr = self.run(cmd, check=False, needs_sudo=True)
        if success:
            if stdout: self.log_debug(f"Sortie fail2ban-client: {stdout.strip()}")
            self.log_debug(f"IP {ip_address} débannie de la jail '{jail}'.")
            return True
        else:
    ...
    def fail2ban_status(self, jail: Optional[str] = None) -> Optional[str]:
        ...
        self.log_debug(f"Récupération du statut fail2ban {target}")
    ...
    def parse_acl(self, acl_output: str) -> Dict[str, Any]:
        """
        ...
        for line in acl_output.splitlines():
            ...
            if len(parts) == 3:
                 ...
                 self.log_warning(f"Ligne ACL non reconnue ignorée: '{line}'")

        self.log_debug(f"ACLs parsées: {acl_data}")
        return acl_data

    def get_acl(self, path: Union[str, Path]) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        target_path = Path(path)
        self.log_debug(f"Récupération des ACLs pour: {target_path}")
        ...
        if not success:
            ...
            if "operation not supported" in stderr.lower():
                 self.log_warning(f"Les ACLs ne semblent pas supportées ou activées pour {target_path}.")
                 # Retourner les permissions de base ? Pour l'instant None.
                 return None
            self.log_error(f"Échec de getfacl pour {target_path}. Stderr: {stderr}")
            return None

        try:
            return self.parse_acl(stdout)
        except Exception as e:
             self.log_error(f"Erreur lors du parsing de la sortie getfacl: {e}", exc_info=True)
    ...
    def set_acl(self,
                ...
                modify: bool = True, # Par défaut, modifie (-m)
        ...
        if clear:
            ...
            action = "Suppression des ACLs spécifiées"
             ...
             self.log_error("Aucune action spécifiée pour set_acl (modify, remove, remove_default, clear).")
             return False
        ...
        cmd.append(str(target_path))
        self.log_debug(f"{action} sur: {target_path}")
        if acl_spec: self.log_debug(f"  Spécification: {spec_to_use}")

        # setfacl nécessite root
        success, stdout, stderr = self.run(cmd, check=False, needs_sudo=True)

        if success:
            self.log_debug(f"ACLs mises à jour avec succès pour {target_path}.")
            return True
        else:
            self.log_error(f"Échec de setfacl sur {target_path}. Stderr: {stderr}")
