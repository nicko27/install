...
class EfiCommands(PluginsUtilsBase):
    ...
    def _check_commands(self):
        ...
        for cmd, attr_name in cmds_to_check.items():
            ...
            if success and path.strip():
                ...
                self.log_debug(f"Commande '{cmd}' trouvée: {path.strip()}", log_levels=log_levels)
            else:
                missing.append(cmd)
                setattr(self, attr_name, None)

        if 'efibootmgr' in missing:
            self.log_error("Commande 'efibootmgr' non trouvée. Ce module ne fonctionnera pas.", log_levels=log_levels)
        if 'findmnt' in missing:
            self.log_warning("Commande 'findmnt' non trouvée. La détection auto de l'ESP échouera.", log_levels=log_levels)

    def _is_efi_system(self) -> bool:
        """Vérifie si le système a démarré en mode EFI en testant l'existence de /sys/firmware/efi."""
        # Pas besoin de sudo pour vérifier l'existence du dossier
        is_efi, _, _ = self.run(['test', '-d', '/sys/firmware/efi'], check=False, no_output=True, needs_sudo=False)
        if not is_efi:
            self.log_warning("Le système ne semble pas démarré en mode EFI (/sys/firmware/efi absent). Les commandes efibootmgr échoueront probablement.", log_levels=log_levels)
        else:
            self.log_debug("Système démarré en mode EFI.", log_levels=log_levels)
        return is_efi
    ...
    def _get_esp_path(self) -> Optional[Path]:
        ...
        if not self._findmnt_path:
            self.log_warning("Commande findmnt non trouvée, impossible de localiser l'ESP automatiquement.", log_levels=log_levels)
            # Essayer les chemins par défaut
            for p in self.ESP_MOUNT_POINTS:
                 path_obj = Path(p)
                 if path_obj.is_dir() and path_obj.is_mount():
                      self.log_info(f"ESP potentiellement trouvé sur (chemin par défaut): {p}", log_levels=log_levels)
                      return path_obj
            self.log_error("Impossible de deviner le chemin de l'ESP.", log_levels=log_levels)
            return None

        # Utiliser findmnt pour chercher le type vfat et le flag 'boot,esp' ou juste vfat sur /boot/efi etc.
        # findmnt -J -t vfat --evaluate : cherche vfat et essaie de trouver la partition esp
                ...
                if 'filesystems' in data:
                    for fs in data['filesystems']:
                        ...
                        if ('esp' in options and 'boot' in options) or target in self.ESP_MOUNT_POINTS:
                            self.log_info(f"Partition ESP trouvée sur: {target} (Source: {fs.get('source')})", log_levels=log_levels)
                            return Path(target)
            except json.JSONDecodeError:
                self.log_warning("Erreur parsing JSON de findmnt pour ESP.", log_levels=log_levels)
            except Exception as e:
                 self.log_warning(f"Erreur inattendue recherche ESP: {e}", log_levels=log_levels)
        ...
        self.log_error("Partition ESP (EFI System Partition) non trouvée ou non montée. Les opérations de création d'entrée peuvent échouer.", log_levels=log_levels)
        return None
    ...
    def _parse_efibootmgr_output(self, output: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            try:
                if line.startswith('BootCurrent:'):
                    ...
                    parsed_data['BootOrder'] = line.split(':')[1].strip().split(',')
                     ...
                     if match:
                          ...
                          self.log_warning(f"Ligne d'entrée EFI non reconnue: '{line}'", log_levels=log_levels)
                ...
                self.log_warning(f"Erreur lors du parsing de la ligne efibootmgr '{line}': {e}", log_levels=log_levels)
    ...
    def list_boot_entries(self, verbose: bool = False, log_levels: Optional[Dict[str, str]] = None) -> Optional[Dict[str, Any]]:
        ...
        self.log_info("Listage des entrées de démarrage EFI (efibootmgr)", log_levels=log_levels)
        ...
        if not success:
            if "efi variables are not supported" in stderr.lower():
                self.log_warning("Le système ne semble pas supporter les variables EFI (ou module efivarfs non chargé).", log_levels=log_levels)
            elif "no such file or directory" in stderr.lower() and "efivars" in stderr.lower():
                 self.log_warning("Système de fichiers efivarfs non monté ou inaccessible.", log_levels=log_levels)
            else:
                self.log_error(f"Échec de efibootmgr. Stderr: {stderr}", log_levels=log_levels)
            return None

        try:
            parsed_data = self._parse_efibootmgr_output(stdout)
            self.log_info(f"{len(parsed_data.get('Entries', {}))} entrées EFI trouvées.", log_levels=log_levels)
            return parsed_data
        except Exception as e:
            self.log_error(f"Erreur lors du parsing de la sortie efibootmgr: {e}", exc_info=True, log_levels=log_levels)
    ...
    def create_boot_entry(self,
                          ...
                          check_esp: bool = True,
                          check_loader: bool = True, log_levels: Optional[Dict[str, str]] = None) -> bool:
        """
        ...
        self.log_info(f"Tentative de création d'une entrée de démarrage EFI '{label}'", log_levels=log_levels)
        self.log_debug(f"  Disque: {disk}, Partition: {partition}, Loader: {loader}")
        self.log_warning("Assurez-vous que la partition EFI spécifiée est correcte et non cryptée.", log_levels=log_levels)
        self.log_warning("Assurez-vous que le chargeur ('loader') spécifié est capable de gérer le système (ex: déchiffrer les disques si nécessaire).", log_levels=log_levels)

        esp_path: Optional[Path] = None
        if check_esp or check_loader:
            esp_path = self._get_esp_path()
            if not esp_path:
                 self.log_error("Vérification ESP échouée. Impossible de continuer sans chemin ESP.", log_levels=log_levels)
                 return False
        ...
        if check_loader and esp_path:
             ...
             self.log_debug(f"Vérification de l'existence du chargeur: {loader_system_path}", log_levels=log_levels)
             ...
             if not loader_exists:
                  self.log_error(f"Le fichier chargeur spécifié '{loader}' ne semble pas exister sur l'ESP ({loader_system_path}).", log_levels=log_levels)
                  return False
             self.log_info(f"Fichier chargeur trouvé: {loader_system_path}", log_levels=log_levels)
        ...
        success, stdout, stderr = self.run(cmd, check=False, needs_sudo=True)
        if success:
             self.log_success(f"Entrée EFI '{label}' créée avec succès.", log_levels=log_levels)
             if stdout: self.log_info(f"Sortie efibootmgr:\n{stdout}", log_levels=log_levels)
             # Suggérer de vérifier/mettre à jour l'ordre de boot
             self.log_info("Pensez à vérifier et ajuster l'ordre de démarrage (BootOrder) si nécessaire via 'set_boot_order'.", log_levels=log_levels)
             return True
        else:
             # Analyser les erreurs courantes
             if "could not prepare boot variable" in stderr.lower():
                  self.log_error(f"Échec création entrée EFI: Impossible de préparer la variable (problème NVRAM ou droits?). Stderr: {stderr}", log_levels=log_levels)
             elif "no space left on device" in stderr.lower():
                  self.log_error(f"Échec création entrée EFI: Espace insuffisant dans la NVRAM. Stderr: {stderr}", log_levels=log_levels)
             elif "invalid partition number" in stderr.lower():
                  self.log_error(f"Échec création entrée EFI: Numéro de partition '{partition}' invalide pour le disque '{disk}'. Stderr: {stderr}", log_levels=log_levels)
             else:
                  self.log_error(f"Échec de la création de l'entrée EFI '{label}'. Stderr: {stderr}", log_levels=log_levels)
             return False
    ...
    def delete_boot_entry(self, boot_num: Union[int, str], log_levels: Optional[Dict[str, str]] = None) -> bool:
        ...
        try:
            ...
            self.log_error(f"Numéro d'entrée EFI invalide: '{boot_num}'. Doit être un entier.", log_levels=log_levels)
            return False

        self.log_warning(f"Suppression de l'entrée de démarrage EFI Boot{boot_num_hex}", log_levels=log_levels)
        cmd = [self._efibootmgr_path, '-b', boot_num_hex, '-B']
        success, stdout, stderr = self.run(cmd, check=False, needs_sudo=True)
        if success:
            self.log_success(f"Entrée EFI Boot{boot_num_hex} supprimée.", log_levels=log_levels)
            if stdout: self.log_info(f"Sortie efibootmgr:\n{stdout}", log_levels=log_levels)
            return True
        else:
            if "could not delete" in stderr.lower() and ("no such file or directory" in stderr.lower() or "invalid argument" in stderr.lower()):
                self.log_warning(f"L'entrée EFI Boot{boot_num_hex} n'existait pas ou numéro invalide.", log_levels=log_levels)
                return True # Considéré comme succès car l'entrée n'est plus là
            self.log_error(f"Échec de la suppression de l'entrée EFI Boot{boot_num_hex}. Stderr: {stderr}", log_levels=log_levels)
            return False
    ...
    def set_boot_order(self, boot_order: List[Union[int, str]], log_levels: Optional[Dict[str, str]] = None) -> bool:
        ...
        try:
            ...
            self.log_error(f"Ordre de démarrage invalide: {boot_order}. Contient des éléments non numériques.", log_levels=log_levels)
            return False

        self.log_info(f"Définition de l'ordre de démarrage EFI: {order_str}", log_levels=log_levels)
        cmd = [self._efibootmgr_path, '-o', order_str]
        success, stdout, stderr = self.run(cmd, check=False, needs_sudo=True)
        if success:
            self.log_success(f"Ordre de démarrage EFI mis à jour: {order_str}.", log_levels=log_levels)
            if stdout: self.log_info(f"Sortie efibootmgr:\n{stdout}", log_levels=log_levels)
            return True
        else:
            # Gérer les numéros invalides
            if "invalid argument" in stderr.lower() or "not found" in stderr.lower():
                 self.log_error(f"Échec: Un ou plusieurs numéros dans l'ordre ({order_str}) sont invalides ou n'existent pas. Stderr: {stderr}", log_levels=log_levels)
            else:
                 self.log_error(f"Échec de la définition de l'ordre de démarrage EFI. Stderr: {stderr}", log_levels=log_levels)
            return False
    ...
    def set_boot_active(self, boot_num: Union[int, str], active: bool = True, log_levels: Optional[Dict[str, str]] = None) -> bool:
        ...
        try:
            ...
            self.log_error(f"Numéro d'entrée EFI invalide: '{boot_num}'.", log_levels=log_levels)
            return False

        action = "Activation" if active else "Désactivation"
        option = '-a' if active else '-A'
        self.log_info(f"{action} de l'entrée de démarrage EFI Boot{boot_num_hex}", log_levels=log_levels)
        cmd = [self._efibootmgr_path, '-b', boot_num_hex, option]

        success, stdout, stderr = self.run(cmd, check=False, needs_sudo=True)
        if success:
            self.log_success(f"Entrée EFI Boot{boot_num_hex} {'activée' if active else 'désactivée'}.", log_levels=log_levels)
            if stdout: self.log_info(f"Sortie efibootmgr:\n{stdout}", log_levels=log_levels)
            return True
        else:
            if "could not" in stderr.lower() and ("no such file or directory" in stderr.lower() or "invalid argument" in stderr.lower()):
                 self.log_error(f"Échec: L'entrée EFI Boot{boot_num_hex} n'existe pas ou est invalide.", log_levels=log_levels)
            else:
                 self.log_error(f"Échec de l'{action.lower()} de l'entrée EFI Boot{boot_num_hex}. Stderr: {stderr}", log_levels=log_levels)
            return False

# Fin de la classe EfiCommands