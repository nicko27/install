...
class UserGroupCommands(PluginsUtilsBase):
    ...
    def _encrypt_password(self, plaintext_password: str, method: Optional[Any] = None) -> Optional[str]:
        ...
        """
        self.log_debug("Cryptage du mot de passe...")
        try:
            ...
            if not encrypted:
                 self.log_error("La fonction crypt.crypt() a retourné une chaîne vide.")
                 return None

            self.log_debug("Mot de passe crypté avec succès.")
            return encrypted
        except Exception as e:
            self.log_error(f"Erreur lors du cryptage du mot de passe: {e}", exc_info=True)
            return None
    ...
    def add_user(self,
        ...
        if self.user_exists(username):
            self.log_info(f"L'utilisateur '{username}' existe déjà.")
            return False
        ...
        if encrypted_password:
             ...
             self.log_info("  - Mot de passe (crypté) fourni via useradd -p.")
        ...
        self.log_success(f"Utilisateur '{username}' ajouté avec succès via useradd.")

        # Définir le mot de passe en clair si fourni (et si pas déjà fait via -p)
        if password_to_set_later:
            self.log_info(f"Définition du mot de passe pour {username} via chpasswd...")
    ...
    def modify_user(self,
        ...
        if set_groups is not None: cmd.extend(['-G', ','.join(set_groups)]); options_added = True; self.log_info(f"  - Définition des groupes: {', '.join(set_groups)}")
    ...
    def set_password(self, username: str, password: str, is_encrypted: bool = False) -> bool:
        ...
        if is_encrypted:
            ...
            if not password_to_send:
                 ...
                 return False
            self.log_info("  - Mot de passe en clair fourni, cryptage avant envoi.")
