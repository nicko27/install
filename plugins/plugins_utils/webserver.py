...
class WebServerCommands(PluginsUtilsBase):
    ...
    def _check_commands(self):
        ...
        for cmd_name in ['apache2ctl', 'apachectl', 'httpd']:
            ...
            if success:
                ...
                self.log_debug(f"Commande Apache trouvée: {self._apache_cmd} (service: {self._apache_service_name})")
                break
        if not self._apache_cmd:
            self.log_debug("Aucune commande Apache (apache2ctl, apachectl, httpd) trouvée.")
        ...
        if success_nginx:
            ...
            self.log_debug(f"Commande Nginx trouvée: {self._nginx_cmd}")
        else:
             self.log_debug("Commande Nginx non trouvée.")
        ...
        for cmd_name in ['a2ensite', 'a2dissite', 'a2enmod', 'a2dismod']:
             ...
             if not success:
                  self.log_debug(f"Commande Apache '{cmd_name}' non trouvée (peut être normal sur non-Debian).")


    def detect_webserver(self) -> str:
        """
        ...
        if apache_active:
             self.log_info("Serveur web détecté: Apache (actif)")
             return self.SERVER_APACHE
        if nginx_active:
             self.log_info("Serveur web détecté: Nginx (actif)")
             return self.SERVER_NGINX

        # Si services non actifs ou non vérifiables, se baser sur la présence des commandes
        if apache_present:
             self.log_info("Serveur web détecté: Apache (commande présente)")
             return self.SERVER_APACHE
        if nginx_present:
             self.log_info("Serveur web détecté: Nginx (commande présente)")
             return self.SERVER_NGINX

        self.log_info("Aucun serveur web principal (Apache/Nginx) détecté.")
        return self.SERVER_UNKNOWN

    # --- Opérations Apache ---

    def apache_check_config(self) -> bool:
        """Vérifie la syntaxe de la configuration Apache."""
        if not self._apache_cmd:
            self.log_error("Commande Apache (apachectl/httpd) non trouvée.")
            return False
        self.log_info("Vérification de la configuration Apache (configtest)")
    ...
    def _run_apache_tool(self, tool: str, target: str, action_verb: str) -> bool:
        ...
        if not success:
             self.log_error(f"Commande Apache '{tool}' non trouvée.")
             return False
        cmd_path = path.strip()

        self.log_info(f"{action_verb.capitalize()} Apache '{target}' via {tool}")
        ...
        if f"{target} {action_past}" in output or "run systemctl reload apache2" in output:
             self.log_success(f"Apache: {target} {action_past} avec succès.")
             self.log_info("Un rechargement/redémarrage d'Apache est nécessaire pour appliquer les changements.")
             return True
        elif f"{target} already enabled" in output or f"{target} déjà activé" in output:
             self.log_info(f"Apache: {target} est déjà {action_past}.")
             return True
        elif f"{target} does not exist" in output or f"n'existe pas" in output:
             self.log_error(f"Apache: {target} n'existe pas.")
             return False
        else:
             self.log_error(f"Échec de '{tool} {target}'. Sortie:\n{output}")
             return False

    def apache_enable_site(self, site_name: str) -> bool:
        """Active un site Apache (ex: '000-default' ou 'my-site.conf')."""
        return self._run_apache_tool('a2ensite', site_name, 'activer site')

    def apache_disable_site(self, site_name: str) -> bool:
        """Désactive un site Apache."""
        return self._run_apache_tool('a2dissite', site_name, 'désactiver site')

    def apache_enable_module(self, module_name: str) -> bool:
        """Active un module Apache."""
        return self._run_apache_tool('a2enmod', module_name, 'activer module')

    def apache_disable_module(self, module_name: str) -> bool:
        """Désactive un module Apache."""
        return self._run_apache_tool('a2dismod', module_name, 'désactiver module')

    def apache_reload(self) -> bool:
        """Recharge la configuration Apache."""
        if not self._apache_service_name:
            self.log_error("Nom du service Apache inconnu, impossible de recharger.")
            return False
        if not self._service_manager:
            self.log_error("Gestionnaire de services non disponible, impossible de recharger Apache.")
            return False
        self.log_info(f"Rechargement du service Apache ({self._apache_service_name})")
        # Utiliser 'reload' qui est plus gracieux que 'restart'
        return self._service_manager.reload(self._apache_service_name)

    def apache_restart(self) -> bool:
        """Redémarre le service Apache."""
        if not self._apache_service_name:
            self.log_error("Nom du service Apache inconnu, impossible de redémarrer.")
            return False
        if not self._service_manager:
            self.log_error("Gestionnaire de services non disponible, impossible de redémarrer Apache.")
            return False
        self.log_info(f"Redémarrage du service Apache ({self._apache_service_name})")
        return self._service_manager.restart(self._apache_service_name)

    # --- Opérations Nginx ---

    def nginx_check_config(self) -> bool:
        """Vérifie la syntaxe de la configuration Nginx."""
        if not self._nginx_cmd:
            self.log_error("Commande Nginx non trouvée.")
            return False
        self.log_info("Vérification de la configuration Nginx (nginx -t)")
        ...
        if "syntax is ok" in stderr_lower and "test is successful" in stderr_lower:
            self.log_success("Syntaxe de configuration Nginx OK.")
            return True
        else:
    ...
    def _get_nginx_sites_dirs(self) -> Tuple[Optional[Path], Optional[Path]]:
        ...
        for base_path in common_paths:
            ...
            if available.is_dir() and enabled.is_dir():
                 self.log_debug(f"Répertoires Nginx trouvés: {available}, {enabled}")
                 return available, enabled
        self.log_error("Impossible de trouver les répertoires 'sites-available' et 'sites-enabled' pour Nginx.")
        return None, None

    def nginx_enable_site(self, site_config_filename: str) -> bool:
        ...
        self.log_info(f"Activation du site Nginx: {site_config_filename}")

        if not site_available_path.is_file():
            self.log_error(f"Fichier de configuration '{site_config_filename}' non trouvé dans {available_dir}.")
            return False
        ...
        if site_enabled_path.is_symlink() or site_enabled_path.exists():
            ...
            if site_enabled_path.is_symlink() and os.readlink(str(site_enabled_path)) == str(site_available_path):
                 self.log_info(f"Site Nginx '{site_config_filename}' déjà activé.")
                 return True
            else:
                 self.log_warning(f"Un fichier/lien existe déjà pour '{site_config_filename}' dans {enabled_dir}. Tentative de remplacement.")
                 ...
                 if not rm_success:
                      self.log_error(f"Impossible de supprimer l'ancien lien/fichier '{site_enabled_path}'. Stderr: {rm_stderr}")
                      return False
        ...
        if success:
            self.log_success(f"Site Nginx '{site_config_filename}' activé.")
            self.log_info("Un rechargement/redémarrage de Nginx est nécessaire.")
            return True
        else:
            self.log_error(f"Échec de l'activation du site Nginx '{site_config_filename}'. Stderr: {stderr}")
            return False

    def nginx_disable_site(self, site_config_filename: str) -> bool:
        ...
        site_enabled_path = enabled_dir / site_config_filename
        self.log_info(f"Désactivation du site Nginx: {site_config_filename}")

        if not site_enabled_path.is_symlink() and not site_enabled_path.exists():
            self.log_info(f"Site Nginx '{site_config_filename}' déjà désactivé ou inexistant.")
            return True
        ...
        if success:
            self.log_success(f"Site Nginx '{site_config_filename}' désactivé.")
            self.log_info("Un rechargement/redémarrage de Nginx est nécessaire.")
            return True
        else:
            self.log_error(f"Échec de la désactivation du site Nginx '{site_config_filename}'. Stderr: {stderr}")
            return False

    def nginx_reload(self) -> bool:
        """Recharge la configuration Nginx."""
        if not self._nginx_service_name:
            self.log_error("Nom du service Nginx inconnu, impossible de recharger.")
            return False
        if not self._service_manager:
            self.log_error("Gestionnaire de services non disponible, impossible de recharger Nginx.")
            return False
        # Vérifier la config avant de recharger
        if not self.nginx_check_config():
            self.log_error("Rechargement annulé en raison d'erreurs de configuration Nginx.")
            return False
        self.log_info(f"Rechargement du service Nginx ({self._nginx_service_name})")
        return self._service_manager.reload(self._nginx_service_name)

    def nginx_restart(self) -> bool:
        """Redémarre le service Nginx."""
        if not self._nginx_service_name:
            self.log_error("Nom du service Nginx inconnu, impossible de redémarrer.")
            return False
        if not self._service_manager:
            self.log_error("Gestionnaire de services non disponible, impossible de redémarrer Nginx.")
            return False
        # Vérifier la config avant de redémarrer
        if not self.nginx_check_config():
            self.log_error("Redémarrage annulé en raison d'erreurs de configuration Nginx.")
            return False
        self.log_info(f"Redémarrage du service Nginx ({self._nginx_service_name})")
