    ...
    def run(self,config,log,target_ip):
        try:
            ...
            if is_ssh==False or (is_good_sms and is_good_lrpgn):
                ...
                steps=4+dovecot_sauvegarde+len(dovecot_admin.split(","))+len(dovecot_modif.split(","))+len(dovecot_user.split(","))
                ...
                if returnValue:
                    try:
                        content=dovecotCmd.read_config("mail")
                        namespace_config={
                            "inbox":"no",
                            "type":"public",
                            "separator":"/",
                        ...
                        returnValue=False
                        output_msg="Erreur dans la lecture du fichier 10-mail.conf"
                if returnValue:
                    log.info("Ajout du namespace")
                    returnValue=dovecotCmd.add_namespace(dovecot_unite,namespace_config=namespace_config,backup="False")
                    if returnValue==False:
                        output_msg="Impossible d'ajouter le namespace"
                    log.next_step()

                if returnValue:
                    log.info("Redémarrage de dovecot")
                    returnValue=servicesCmd.restart("dovecot")
                    log.next_step()
                # Résultat final
                if returnValue:
                    output_msg = "Dovecot installé et configuré avec succès"
                else:
                    output_msg="Impossible de redémarrer dovecot"
            else:
