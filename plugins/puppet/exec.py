    ...
    def run(self,config,log,target_ip):
        try:
            ...
            if is_ssh==False or (is_good_sms and is_good_lrpgn):
                ...
                returnValue,stdout,stderr=metierCmd.run("/bin/bash /usr/local/sbin/puppet-contact --force", needs_sudo=True)
                returnValue=False
                for line in stdout.split('\n'):
                    if "Contact Puppet [OK]" in line.strip():
                        returnValue=True

                # Résultat final
                if returnValue:
                    output_msg = "Puppet mis à jour avec succès"
                else:
                    output_msg = "Problème avec la mise à jour Puppet"
            else:
